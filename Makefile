...
APP_TITLE   :=  JKSV
APP_AUTHOR  :=  JK
APP_VERSION :=  $(shell git log -1 --pretty=format:%as HEAD)
ROMFS	    :=	romfs
ICON		:=	icon.jpg
...
LIBS	:= `sdl2-config --libs` `freetype-config --libs` `curl-config --libs` -lSDL2_image -lwebp -lpng -ljpeg -lz -lminizip -lmbedcrypto -ljson-c -lnx
